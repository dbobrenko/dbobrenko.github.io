...
excerpt: "I'll implement and explain an idea of Asynchronous one-step Q-Learning. As an example will be train an agent to play in classic Atari SpaceInvaders and Pong games, using only raw pixel observations!" 
date: 2016-11-03 22:00:00 
mathjax: true
---

Deep Reinforcement Learning has recently became a really hot area of research, due to the huge amount of breakthroughs in last couple of years. Such "explosion" started by a group of scientists from a start-up company called DeepMind (later it was acquired by Google), who decided to apply current deep learning progress to existing reinforcement learning (RL) approaches. The result paper [Playing Atari with Deep Reinforcement Learning", Mnih et al., 2013](https://arxiv.org/abs/1312.5602) recieves a lot of attention in AI community, since it is the first time, when a single algorithm, using only raw pixel observations, successfully learns how to survive in absolutely different evironments, with different rules and objectives, and in some of the games, it even outperforms human!
Many improvements have been made to Deep Q-Network (DQN) since 2013, few of them: Double DQN, Dueling DQN, Dueling Double DQN. In this topic we will implement Google DeepMind's asynchronous one-step Q-Learning method, presented in [Asynchronous Methods for Deep Reinforcement Learning, Mnih et al., 2016.](https://arxiv.org/abs/1602.01783), with classic Atari 2600 games (however it can work with any OpenAI Gym environment with raw visual input). 
...
For implementation was used deep learning [TensorFlow](http://tensorflow.org) and [Keras](https://keras.io/) libraries.
...
For impatient, you can download pretrained agent from [here](**TODO link to the model**). The model was trained asynchronously in 8 threads over 30 hours on GTX 980 Ti GPU, in total of 30 millions of frames (however it can be trained further).
...
1. Why does the future rewards are important? Can't we just take into account only immediate rewards ($\gamma = 0$)?
...
2. Then why we just dont take **total discounted future reward** ($\gamma = 1$)?
...
Since DQN learns to predict continuous reward values for each action in the action space - it can be interpreted as a regression task.
That's why we will define mean squared error **loss function** for our neural network:
...
*Keep in mind, that some games has "rounds" (most Atari games do), in order to avoid repeating actions from last game in new one, and not to predict expected future rewards for current game, based on state from the next game, we should handle end of these rounds as terminal states*
...
->Figure 4: Average reward per episode along the training on SpaceInvaders.<-

->![alt text][image_q_plot_si]<-
->Figure 5: Average predicted Q-values per episode along the training on SpaceInvaders.<-

**Visualization.**
->![alt text][image_filter_vis]<-
->Figure 6: Filter visualization of the model trained on SpaceInvaders.<-

->![alt text][image_q_values]<-
->Figure 7: Model's Q-values prediction for given input state.<-
...
To get more intuition about classic Deep Q-Network you may read through [this post](https://www.nervanasys.com/demystifying-deep-reinforcement-learning), simultaneously watching [10 David Silver's lectures about RL](http://www0.cs.ucl.ac.uk/staff/d.silver/web/Teaching.html)
...
After, watch and read through [CS231n Stanford lectures](http://cs231n.github.io/) (unfortunately official video lectures were removed from youtube, but probably, somewhere, there might be an unofficial one ;) ).
...
3. Deep Policy Gradients: [Continuous control with deep reinforcement learning, Lillicrap, Hunt et al., 2016](http://arxiv.org/pdf/1509.02971v5.pdf).
...
[image_onestep_alg]: /assets/posts/async-deeprl/onestep_alg.png "Asynchronous Q-Learning algorithm pseudo-code"
[image_reward_plot_si]: /assets/posts/async-deeprl/si36_reward.png "Average episode rewards along training"
[image_q_plot_si]: /assets/posts/async-deeprl/si36_q.png "Average Q-value prediction progress along training"
[image_filter_vis]: /assets/posts/async-deeprl/inprogress.jpg "Trained on SpaceInvaders model filter visualization"
[image_q_values]: /ssets/posts/async-deeprl/inprogress.jpg "Trained on SpaceInvaders model Q-values prediction"